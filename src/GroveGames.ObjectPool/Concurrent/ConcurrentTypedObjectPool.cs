...
public sealed class ConcurrentTypedObjectPool<TBase, TDerived> : IObjectPool<TBase> where TDerived : class, TBase where TBase : class
{
    ...
    public int Count => _disposed ? throw new ObjectDisposedException(nameof(ConcurrentTypedObjectPool<TBase, TDerived>)) : _pool.Count;
    public int MaxSize => _disposed ? throw new ObjectDisposedException(nameof(ConcurrentTypedObjectPool<TBase, TDerived>)) : _pool.MaxSize;
